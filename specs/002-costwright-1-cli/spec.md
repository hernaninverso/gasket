# Feature Specification: costwright v0.1 — el wedge OSS (CLI certificador de presupuesto)

**Feature Branch**: `002-costwright-1-cli`
**Created**: 2026-06-12
...
es el respaldo formal y el canal de distribución — **costwright se lanza JUNTO al paper** (decisión de
Hernán 2026-06-12). El negocio (certificación firmada, feed de billing, capa org) queda FUERA del
OSS — esta spec construye SOLO el wedge. Licencia Apache-2.0, repo público `hernaninverso/costwright`.
...
### User Story 1 - `costwright check .` emite el veredicto (Priority: P1)

Un dev con un repo LangGraph corre `pip install costwright && costwright check .` y en <30s ve, por cada
...
**Independent Test**: correr `costwright check` sobre 3 repos del dataset congelado de 001 reproduce
las categorías de results.json.

**Acceptance Scenarios**:

1. **Given** un repo con grafos, **When** `costwright check .`, **Then** salida humana con tabla por
...
### User Story 2 - Inferencia de caps (`costwright caps`) (Priority: P2)

El 88.1% depende de defaults y solo 11.9% anota caps. `costwright caps .` lista cada constructor de
...
1. **Given** `ChatOpenAI(model=...)` sin cap, **When** `costwright caps`, **Then** sugiere el kwarg
...
`uses: hernaninverso/costwright-action@v1` corre `costwright check --json` y falla el build según política
...
- **FR-001**: `costwright check [path]` — recicla extractor/mapper de 001 (mismo núcleo, mismos tests);
  salida pretty (rich-free, stdlib only) + `--json` (schema `costwright.v1`); exit codes 0/1/2 (US1).
- **FR-002**: `costwright caps [path]` — detección de constructores LLM sin cap + sugerencia por
  provider según la tabla §3.2 VERIFICADA; `--fix` opt-in con edit idempotente y hash-check.
- **FR-003**: Packaging: `pyproject.toml`, `pip install costwright`, entrypoint `costwright`, Python ≥3.10,
...
- **FR-008**: CI del repo: GitHub Actions corriendo tests en 3.10/3.12/3.13 + el propio costwright
  sobre sí mismo (dogfood, debe salir exit 0).

### Key Entities

- **CheckResult** (= MappingResult de 001 + presentación), **CapFinding** (constructor, provider,
  kwarg correcto, span, fix sugerido), **CertReport** (schema costwright.v1).

## Success Criteria *(mandatory)*

- **SC-001**: sobre el dataset congelado de 001, `costwright check` reproduce las 254 categorías
  exactas de results.json (cero drift con el experimento).
- **SC-002**: `costwright caps` encuentra ≥90% de los 123 constructores LLM detectados en 001 y NO
  sugiere kwargs incorrectos para los 4 providers de la tabla (validado contra fixtures por provider).
- **SC-003**: `pip install` desde sdist limpio en venv nuevo funciona; `costwright check` corre <30s
...
- El naming "costwright" en PyPI: verificar disponibilidad ANTES de fijar el nombre del package; si está
  tomado → `costwright` como package con CLI `costwright` (decisión técnica mía, no re-preguntar).
...
1. **Exit codes (REEMPLAZA US1.1)**: `costwright check` default = **exit 0 SIEMPRE que el tool corra**
...
2. **`--fix` ELIMINADO; reemplazo: `costwright caps --patch`** emite unified diff a stdout/archivo
...
3. **Schema `costwright.v1` CONGELADO**: `{schema:"costwright.v1", units:[{unit_id(hash estable),
