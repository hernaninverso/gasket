# Tasks: costwright fusion (003)

> Orden dead-code-first: schema+bundler → CLI → demo → tests → docs → audit-3.

## Fase 1 — Bundler (stdlib, zero-dep)
- [ ] T1.1 `src/costwright/fusion.py`: `SCHEMA`, `canonical`, `digest`.
- [ ] T1.2 `cost_certificate(costwright_v1_report)`: deriva `worst_category`/`status`, `report_digest`,
      embeb `theorem` + `scope`.
...
## Fase 2 — CLI
- [ ] T2.1 `costwright fuse` en `cli.py`: flags (FR-007), lee/valida JSON, `--workflow` → workflow_digest,
      exit 0/2.

## Fase 3 — Demo e2e
- [ ] T3.1 `examples/workflows/certifiable_graph.py` (fixture LangGraph con recursion_limit).
- [ ] T3.2 `examples/fusion_demo.py`: costwright check (vía API interna) + base sintética (overlap) +
...
- [ ] T4.1 `tests_pkg/test_fusion.py` (script-style): golden schema, tamper-evidence, validación
      ...
      derivation, CLI e2e `costwright fuse`.
...
## Fase 6 — Gate
- [ ] T6.1 Correr `test_fusion.py` + `test_cli.py` + dogfood `costwright check src --fail-on reject` verdes.
