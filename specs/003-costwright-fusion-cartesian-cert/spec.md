# Feature Specification: costwright fusion — el "producto cartesiano de certificados" (Exp #4)

**Feature Branch**: `003-costwright-fusion-cartesian-cert`
**Created**: 2026-06-13
...
Exp #4 "Fusión costwright × eleata-verify"). Decisión heredada del council del roadmap: **"DEFER el
...
1. **"¿Me va a reventar el presupuesto?"** → la responde `costwright` con un **certificado de COSTO**:
...
- **costwright core = CERO dependencias runtime** (principio del proyecto, spec 002 FR-003). Por lo tanto
  la fusión es un **bundler de schema en stdlib puro**: toma dos `dict` (un `costwright.v1` y un
  `VerifyResult.to_dict()`) y emite un `costwright.fusion.v1`. **NUNCA importa `eleata_verify`** — solo
...
### User Story 1 — `costwright fuse` emite el certificado combinado (Priority: P1)

Un equipo que ya corre `costwright check . --json > cost.json` en CI y produce un veredicto de riesgo
con `eleata-verify` (`risk.json`) corre `costwright fuse --cost cost.json --risk risk.json --run-id <id>`
y obtiene un único `costwright.fusion.v1` que lleva los dos certificados, sus dos scopes, los digests de
binding y el disclaimer de no-composición.

**Why this priority**: es el entregable central del Exp #4 — el artefacto auditable.

**Independent Test**: dado un `cost.json` y un `risk.json` fijos, `costwright fuse` produce un
`costwright.fusion.v1` que matchea un golden, con `composition.joint_guarantee == false` SIEMPRE.

**Acceptance Scenarios**:

1. **Given** un `cost.json` (costwright.v1) + un `risk.json` (VerifyResult dict) válidos, **When**
   `costwright fuse --cost ... --risk ... --run-id R`, **Then** salida `costwright.fusion.v1` con bloques
...
Un script `examples/fusion_demo.py` corre el flujo COMPLETO offline y determinista: (a) `costwright check`
...
1. **Given** el demo corrido en una venv con `eleata-verify` pineado + `numpy`, **Then** imprime dos
   bundles `costwright.fusion.v1` (answered + abstained), exit 0, determinista (mismo seed → misma salida).
...
- **FR-001**: `src/costwright/fusion.py` (stdlib only) expone: `SCHEMA="costwright.fusion.v1"`,
  `cost_certificate(costwright_v1_report) -> dict`, `risk_certificate(verify_result_dict, *, verify_version,
...
- **FR-004**: Versiones + scopes (council P0): `cost.costwright_version`, `risk.verify_version`;
...
- **FR-007**: CLI `costwright fuse` (stdlib): `--cost FILE --risk FILE --run-id ID
  [--costwright-version V] [--verify-version V] [--workflow PATH] [--calibrator-digest D] [--claim-file F]
...
- **FR-011**: El bundle OSS es informativo, NO firmado (`signature: null` reservado, igual que
  `costwright.v1`). La firma criptográfica del bundle es la capa paga (certd/E1), fuera de este repo.
...
- **CostCertificate** (bloque `cost`): deriva del `costwright.v1` report + provenance del teorema.
- **RiskCertificate** (bloque `risk`): deriva del `VerifyResult.to_dict()` + metadata del calibrador.
- **FusionBundle** (`costwright.fusion.v1`): el producto cartesiano + `composition` (el límite de honestidad).

## Success Criteria *(mandatory)*

- **SC-001**: `fuse()` sobre fixtures fijos reproduce un golden `costwright.fusion.v1` byte-a-byte
...
- La capa firmada (certd/E1) que firma el `costwright.fusion.v1` NO está en scope; solo se deja el hook
  `signature: null` (paridad con `costwright.v1`).
...
4. **Versiones de ambos + scopes temporales explícitos (P0)**: `costwright_version`, `verify_version`,
