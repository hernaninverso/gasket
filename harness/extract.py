...
if __name__ == "__main__":
    ...
    out = [extract_unit(ROOT / "dataset" / "frozen" / m["unit_id"], m) for m in man]
    (ROOT / "results").mkdir(exist_ok=True)
    (ROOT / "results" / "extraction.json").write_text(json.dumps(out, indent=1, ensure_ascii=False))
    ok = sum(1 for x in out if x["status"] == "ok")
    print(f"extraídos {ok}/{len(out)}")
